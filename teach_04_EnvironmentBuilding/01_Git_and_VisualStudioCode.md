...
    (特に名前に指定はないです。プログラマーの人々はWindows用のKeyとMac用のKeyと分かりやすくため「Windows」「Mac」というように名前を付けると聞いたことがあります。)

    1. Keyに公開鍵を貼り付ける。  

    ターミナルにて、Windowsでは`clip < ~/.ssh/id_rsa.pub`、Macでは`pbcopy < ~/.ssh/id_rsa.pub`を実行することで、クリップボードにコピーできます。  
...
(※branchを新たに作ることをbranchを***切る**といいます。)  

  

ブランチの名前については、上手い名前の付け方を模索してほしいところでありますが、候補をあげておきます。  
